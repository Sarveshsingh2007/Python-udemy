...
load_dotenv(r"Python-udemy\day70-a\secret.env")
...
def contact():
    if request.method == "POST":
        ...
        return render_template(
            "contact.html", msg_sent=True, current_user=current_user
        )
...
def send_email(name, email, phone, message):
    ...
    with smtplib.SMTP("smtp.gmail.com") as connection:
        ...
        # send the email to yourself (and optionally the user)
        connection.sendmail(
            from_addr=MAIL_ADDRESS,
            to_addrs=[MAIL_ADDRESS],  # or [MAIL_ADDRESS, email] to also send to user
